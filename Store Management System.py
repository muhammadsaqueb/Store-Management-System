...
#import pandas as pd
import datetime
mydb=mysql.connector.connect(host="localhost",user="root",passwd="2251824",database="fashion",charset="utf8")
...
def AddProduct():
    ...
    L.append(sn)
    rate=input("Enter the Rates for Product :")
    L.append(rate)
    product=(L)
    print(L)
    sql="Insert into product values('"+pid+"','"+IName+"','"+brnd+"','"+fr+"','"+sn+"','"+rate+"'); "
    print(sql)
...
def ViewPurchases():
    ...
    sql="select product.product_id","product.PName,product.brand"','"purchase.no_of_items"','"purchase.purchase_date"','"purchase.amount from product INNER JOIN purchase ON product.product_id=purchase.item_id and product.PName='"+item+"';"
    itm=[item]
    mycursor.execute(sql)
    res=mycursor.fetchall()
    for x in res:
...
print("* * * Under The Guidance , Support And supervision Of Mr. Sunil Kumar * * * ")
print("*"*80)
print("")
